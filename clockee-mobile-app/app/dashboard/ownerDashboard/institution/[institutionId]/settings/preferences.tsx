...
export default function InstitutionPreferences() {
 ...
 const fetchSettings = async () => {
  try {
    ...
    // ✅ NO SETTINGS YET
    if (!data) {
      console.log("⚠️ No settings found yet. Using defaults.");
      return;
    }
    ...
    setSettings({
      ...
      enableLateAlerts:
        data.notifications?.enableLateAlerts ?? false,
      enableOutOfZoneAlerts:
        data.notifications?.enableOutOfZoneAlerts ?? false,
    });
    ...
    // ✅ IMPORTANT FIX
    // If settings do not exist yet, DON'T block UI
    if (
      error?.response?.data?.message ===
      "Institution settings not found"
    ) {
      console.log("⚠️ No settings document yet. Continuing...");
      return;
    }
    ...
    <>
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <Section title="Time & Attendance">
          ...
          <TimeInput label="Work Start Time" displayValue={formatDisplayTime(workStartTime, use12HourFormat)} onPress={() => setShowStartPicker(true)} />
          {showStartPicker && (
          ...
          <TimeInput label="Work End Time" displayValue={formatDisplayTime(workEndTime, use12HourFormat)} onPress={() => setShowEndPicker(true)} />
          {showEndPicker && (
